...
describe AllMemberEvent do
  let!(:all_member_event) { FactoryBot.create(:all_member_event) }
  ...
  describe ".event_happening?" do
    it "returns true if the current time is during an all day event" do
      FactoryBot.create(:all_member_event, :scheduled => '1/10/2013 12:00 am')
    ...
    it "returns true if the current time is between 1 hour before and midnight on the day of a scheduled event" do
      FactoryBot.create(:all_member_event, :scheduled => '1/10/2013 01:00 pm')
    ...
    it "returns false if the current time is not during an all day event or 1 hour before a scheduled event" do
      FactoryBot.create(:all_member_event, :scheduled => '1/09/2013 12:00 am')
      FactoryBot.create(:all_member_event, :scheduled => '1/10/2013 02:00 pm')
      FactoryBot.create(:all_member_event, :scheduled => '1/11/2013 12:00 am')
